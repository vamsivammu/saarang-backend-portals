...
/**
 * @ngdoc function
 * @name erpSaarangFrontendApp.controller:EventRegistrationsCtrl
 * @description
...
.config(function ($stateProvider) {
    $stateProvider
      .state('event_registration', {
        url: '/events_r/:name',
        templateUrl: 'views/event_registrations.html',
        controller: 'EventRegistrationsCtrl',
        authenticate: true,
        department: "Hospitality"
      });
  })
  .controller('EventRegistrationsCtrl',['$scope','$http','$stateParams','$localStorage','$state','$mdToast', '$mdDialog', function ($scope,$http,$stateParams,$localStorage,$state,$mdToast,$mdDialog) {
    $scope.sortReverse = false;
    $scope.searchTag = "";
    $scope.allregs = {};
    $scope.disableLeader = false;
    $scope.selected_event = $localStorage.selected_event;
      ...
      $http({
        ...
        data:{
          ...
          "args": {
                        ...
                        "where": {
                          "event_id": {
                              "$eq": $localStorage.selected_event.id
                          }
                      }
                      }
              },
      ...
      $http({
        ...
        data:{
                ...
                "args":{
                        ...
                        "where": {
                          "$and": [
                          {"event_id": $localStorage.selected_event.id},
                          {"team_id": null},
                        ]}
                      }
              },
